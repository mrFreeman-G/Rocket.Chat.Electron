...
const buttonClassName =
  'rcx-box rcx-box--full rcx-box--animated rcx-button--small rcx-button--primary rcx-button add-folder-button';
...
function addChatToLocalStorageChatFolder(folderName, chatName) {
  ...
  if (
    ...
    chatFolders[folderName].indexOf(chatName) === -1
  ) {
...
function updateThemeSettingsHtml(data) {
  ...
  root.setAttribute(
    'data-messages-background-opacity',
    data['messages-background-opacity']
  );
  if (data['messages-background-opacity']) {
    root.style.setProperty(
      '--messages-background-opacity',
      data['messages-background-opacity'] / 100
    );
  }
}
...
function handlePopups() {
  window.addEventListener('click', ({ target }) => {
    ...
    const clickedOnClosedPopup =
      popupButton && !popupButton.classList.contains('show');
  ...
  importSettingsForm.addEventListener('submit', function (e) {
    ...
    fr.onload = function (e) {
      ...
      try {
        ...
        if (settings['foldersSettings'] === undefined)
          throw new Error('Incorrect settings file.');
      ...
      for (let [folderName, chatsList] of Object.entries(
...
function changeFolder(folderName) {
  ...
  const selectedFolderArea = document.querySelector(
...
function handleChangeFolderEvent(e) {
  const folderLabel = e.target
    .closest('div[folder-label]')
    .getAttribute('folder-label');
...
function setupFolderButton(foldersContainer, folderName) {
  if (folderName == settingsLabel)
    return setupThemeSettingsButton(foldersContainer);
  ...
  const mainFolderButton = document.querySelector(
    `div.sidebar-folder[folder-label="${allChatsLabel}"]`
  );
  const personalChatsFolderButton = document.querySelector(
    `div.sidebar-folder[folder-label="${personalChatsLabel}"]`
  );
...
function chatsUnreadStatusDataDivSetup() {
  ...
  let observer = new MutationObserver(function (mutations) {
    if (generalFolderUnreadChats) {
      let unreadChats = Object.values(generalFolderUnreadChats).filter(
        (v) => v === true
      ).length;

      // set counter
      let folderBadge = document.querySelector(
        `div[folder-label="${allChatsLabel}"] span.unread-chats-counter`
      );
      if (unreadChats == 0) {
  ...
  observer.observe(chatsUnreadStatusDataDiv, {
    attributes: true,
    childList: true,
    characterData: true,
    subtree: true,
...
function setupAllChatsFolderArea() {
  const defaultChatsContainer = document.querySelector(
    'nav.rcx-sidebar div[aria-label][role="navigation"]'
  ); // new
  const defaultFolderArea = defaultChatsContainer.querySelector(
    'div:has( [tabindex="0"])'
  );
...
function setupThemeSettingsArea() {
  ...
  const checked = (param, value) => {
    return themeSettings[param] == value ? 'checked="checked"' : '';
  };
  const getThemeParam = (param, defaultValue) => {
    return themeSettings[param] ? themeSettings[param] : defaultValue;
  };
  ...
  let settingsFormHtml = `
		<form id="form-theme-settings">
			...
			<fieldset>
				...
				<div>
					<label for="theme-rocketchat" class="big-radio">
						<input type="radio" id="theme-rocketchat" name="theme" value="rocketchat" ${checked(
              'theme',
              'rocketchat'
            )} />
						RocketChat
				...
				<div>
					<label for="theme-flat" class="big-radio">
						<input type="radio" id="theme-flat" name="theme" value="flat" ${checked(
              'theme',
              'flat'
            )} />
						Flat
				...
				<div>
					<label for="theme-telegram" class="big-radio">
						<input type="radio" id="theme-telegram" name="theme" value="telegram" ${checked(
              'theme',
              'telegram'
            )} />
						Telegram
			...
			<fieldset>
				...
				<div>
					<input type="radio" id="bg-2" name="bg" value="bg-2" ${checked(
            'bg',
            'bg-2'
          )} disabled />
				...
				<div>
					<input type="radio" id="bg-3" name="bg" value="bg-3" ${checked(
            'bg',
            'bg-3'
          )} disabled />
			...
			<fieldset>
				...
				<div>
					<input type="radio" id="palette-light-1" name="palette-light" value="palette-green" ${checked(
            'palette-light',
            'palette-green'
          )}/>
				...
				<div>
					<input type="radio" id="palette-light-2" name="palette-light" value="palette-blue" ${checked(
            'palette-light',
            'palette-blue'
          )}/>
				...
				<div>
					<input type="radio" id="palette-light-3" name="palette-light" value="palette-orange" ${checked(
            'palette-light',
            'palette-orange'
          )}/>
			...
			<fieldset>
				...
				<div>
					<input type="radio" id="palette-dark-1" name="palette-dark" value="palette-dark" ${checked(
            'palette-dark',
            'palette-dark'
          )}/>
				...
				<div>
					<input type="radio" id="palette-dark-2" name="palette-dark" value="palette-darker" ${checked(
            'palette-dark',
            'palette-darker'
          )}/>
		    ...
		    <fieldset>
		      ...
		      <div style="justify-content: center;">
		        <input style="width: 100%;" type="range" min="50" max="100" name="messages-background-opacity" value="${getThemeParam(
              'messages-background-opacity',
              100
...
function removeFolder(folderName) {
  ...
  let folderButton = document.querySelector(
    `div.sidebar-folder[folder-label="${folderName}"]`
  );
  let folderContainer = document.querySelector(
    `div.sidebar-folder-container[folder-label-area="${folderName}"]`
  );
  folderButton.remove();
...
function addChatToFolder(FolderName, chatName) {
  ...
  if (success) {
    let folderDiv = document.querySelector(
      `div.sidebar-folder-container[folder-label-area="${FolderName}"]`
    );
    let mainFolderDiv = document.querySelector(
      `div[folder-label-area="${allChatsLabel}"]`
    );
    let chatElem = mainFolderDiv.querySelector(
      `div[data-index]:has(a[aria-label="${chatName}"])`
    );
...
function removeChatFromFolder(FolderName, chatName) {
  ...
  if (success) {
    ...
    let chatElem = folderDiv.querySelector(
      `div[data-index] a[aria-label="${chatName}"]`
    );
...
function setupFolderManageButtons() {
  ...
  [
    addCurrentChatButton,
    deleteCurrentChatButton,
    deleteCurrentFolderButton,
  ].forEach((button) => {
    button.className =
      'rcx-box rcx-box--full rcx-button--small rcx-button--primary rcx-button';
    manageFolderDiv.append(button);
    button.addEventListener('click', (e) => {
      let action = e.target.id;
      let currentFolder = e.target.closest('.sidebar-folder-container');
      let currentFolderName = currentFolder
        ? currentFolder.getAttribute('folder-label-area')
        : null;
...
function formatExtendedNavbarItems(item) {
  ...
  if (isChat) {
    ...
    let chatsUnreadStatusDataDiv = document.querySelector(
      '.chats-unread-status-data'
    );
    if (chatsUnreadStatusDataDiv) {
      chatsUnreadStatusDataDiv.innerHTML = JSON.stringify(
        generalFolderUnreadChats
      );
    }
  }
...
function formatStickerMessages() {
  let notFormattedStickerMessages = document.querySelectorAll(
    '.rcx-message:not(.is-sticker-message)'
  );
  notFormattedStickerMessages.forEach((message) => {
    let isSequential = Array.from(message.classList).includes(
      'rcx-message--sequential'
    );
    let messageContainer = message.querySelector(
      '.rcx-message-container:not(.rcx-message-container--left)'
    );
    ...
    let withReactions = !!messageContainer.querySelector(
      '.rcx-message-reactions__container'
    )
      ? 1
      : 0;
    let withThread = !!messageContainer.querySelector(
      '.rcx-message-metrics__content-item'
    )
      ? 1
      : 0;
    let withMessageReadReceipt = !!messageContainer.querySelector(
      'i.rcx-icon--name-check'
    )
      ? 1
      : 0;
    ...
    if (
      // in sequential
      ((isSequential &&
        messageContainer.childElementCount ===
          childElementByCondition['cond_1']) ||
        // in sequential with reactions
        (isSequential &&
          withReactions &&
          messageContainer.childElementCount ===
            childElementByCondition['cond_2']) ||
        // in NOT sequential
        (!isSequential &&
          messageContainer.childElementCount ===
            childElementByCondition['cond_3']) ||
        // in NOT sequential with reactions
        (!isSequential &&
          withReactions &&
          messageContainer.childElementCount ===
            childElementByCondition['cond_4'])) &&
...
async function setupNavbar(navbarItemsContainer) {
  ...
  const sidebarWidthMd =
    getComputedStyle(root).getPropertyValue('--sidebar-md-width');
  const sidebarWidthLg =
    getComputedStyle(root).getPropertyValue('--sidebar-lg-width');
  const sidebarFoldersWidth = getComputedStyle(root).getPropertyValue(
    '--sidebar-folders-width'
  );
  const sidebarAdditionalWidth = 0; // TODO

  // set sidebar width
  const newSidebarWidth = `calc(${sidebarWidth} + ${sidebarFoldersWidth}) + ${sidebarAdditionalWidth}`;
  const newSidebarWidthMd = `calc(${sidebarWidthMd} + ${sidebarFoldersWidth}) + ${sidebarAdditionalWidth}`;
  const newSidebarWidthLg = `calc(${sidebarWidthLg} + ${sidebarFoldersWidth}) + ${sidebarAdditionalWidth}`;
  ...
  for (let [folderName, itemLabels] of Object.entries(chatFolders)) {
    ...
    for (let item of navbarItems) {
      ...
      if (
        elemHref.indexOf('direct/') !== -1 &&
        personalChats.indexOf(item) === -1
      ) {
  ...
  navbarItems.forEach((element) => {
    formatExtendedNavbarItems(element);
  });
...
function countFolderUnreadedChats(folderContainer, isGeneralFolder) {
  ...
  allChats.forEach((element) => {
    /*
		 ...
		 */
    let isUnreadedChat = element.querySelectorAll(
  ...
  // set counter
  let folderName = isGeneralFolder
    ? allChatsLabel
    : folderContainer.getAttribute('folder-label-area');
  let folderBadge = document.querySelector(
    `div[folder-label="${folderName}"] span.unread-chats-counter`
  );
...
function countFolderUnreadedChatsInitial(navbarItemsContainer) {
  ...
  for (let [folderName, unreadedChatsCount] of Object.entries(
    foldersUnreadedChatsCount
  )) {
    let folderBadge = document.querySelector(
      `div.sidebar-folder[folder-label="${folderName}"] span.unread-chats-counter`
    );
...
function syncNavbarItemsToFoldes(mutation) {
  ...
  if (sourceElem) {
    ...
    let sourceElementAriaLabel = sourceElementAnchor
      ? sourceElementAnchor.getAttribute('aria-label')
      : null;
    let folderElems = document.querySelectorAll(
      'div.sidebar-folder-container div[data-index]'
    );
    folderElems.forEach((element) => {
      let elementAnchor = element.querySelector('a[aria-label]');
      let elementAriaLabel = elementAnchor
        ? elementAnchor.getAttribute('aria-label')
        : 'null';
...
function startObserveNavbarItems(navbarItemsContainer) {
  ...
  observer.observe(navbarItemsContainer, {
    attributes: true,
    childList: true,
    characterData: true,
    subtree: true,
...
function startObserveFolderItems(folderItemsContainer, isGeneralFolder) {
  ...
  observer.observe(folderItemsContainer, {
    attributes: true,
    childList: true,
    characterData: true,
    subtree: true,
...
async function start() {
  ...
  async function getSideBarElems() {
    ...
    if (navbar) {
      let navbarItemsContainer = navbar.querySelector(
        "div[style*='box-sizing']"
      );
      let navBarItems = navbar.querySelectorAll(
        'div[data-index]:has(a.rcx-sidebar-item)'
      );
